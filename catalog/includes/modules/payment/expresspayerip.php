...
/* -----------------------------------------------------------------------------------------
  ...
  class expresspayerip{
    var $code, $title, $description, $enabled;

// class constructor
    function expresspayerip() {
      global $order;

      $this->code = 'expresspayerip';
      $this->title = MODULE_PAYMENT_EXPRESSPAYERIP_TEXT_TITLE;
      $this->public_title = MODULE_PAYMENT_EXPRESSPAYERIP_TEXT_PUBLIC_TITLE;
      $this->description = MODULE_PAYMENT_EXPRESSPAYERIP_TEXT_ADMIN_DESCRIPTION;
      $this->icon = "https://express-pay.by/Media/Images/Logo.png";
      $this->sort_order = MODULE_PAYMENT_EXPRESSPAYERIP_SORT_ORDER;
      $this->enabled = ((MODULE_PAYMENT_EXPRESSPAYERIP_STATUS == 'True') ? true : false);

      ((MODULE_PAYMENT_EXPRESSPAYERIP_ISTEST == 'True') ? $this->form_action_url = 'https://sandbox-api.express-pay.by/v1/web_invoices' : $this->form_action_url = 'https://api.express-pay.by/v1/web_invoices');
    }
...
// class methods
    function update_status() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_EXPRESSPAYERIP_ZONE > 0) ) {
        $check_flag = false;
        $check_query = vam_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_EXPRESSPAYERIP_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
    ...
    function selection() {
      ...
      if (isset($_SESSION['cart_expresspayerip_id'])) {
        $order_id = substr($_SESSION['cart_expresspayerip_id'], strpos($_SESSION['cart_expresspayerip_id'], '-')+1);
        ...
        if (vam_db_num_rows($check_query) < 1) {
          ...
          unset($_SESSION['cart_expresspayerip_id']);
        }
      }
    ...
    function confirmation() {
      ...
      if (isset($_SESSION['cartID'])) {
        ...
        if (isset($_SESSION['cart_expresspayerip_id'])) {
          ...
          if ( ($curr['currency'] != $order->info['currency']) || ($_SESSION['cartID'] != substr($_SESSION['cart_expresspayerip_id'], 0, strlen($_SESSION['cartID']))) ) {
            $check_query = vam_db_query('select orders_id from ' . TABLE_ORDERS_STATUS_HISTORY . ' where orders_id = "' . (int)$order_id . '" limit 1');
...
//PIN				
          ...
          $_SESSION['cart_expresspayerip_id'] = $_SESSION['cartID'] . '-' . $insert_id;
        }
      }

      return array('title' => MODULE_PAYMENT_EXPRESSPAYERIP_TEXT_DESCRIPTION);
    }
    ...
    function process_button() {
      ...
      $process_button_string = '';
			...
			#номер заказа
			$order_no = substr($_SESSION['cart_expresspayerip_id'], strpos($_SESSION['cart_expresspayerip_id'], '-')+1);

			#получаем стоимость заказа в белорусских рублях
      $sum = number_format($order->info["total"], 2, ',', '');
      
      $request_params = array(
        "ServiceId" => MODULE_PAYMENT_EXPRESSPAYERIP_SERVICEID,
        "AccountNo" => $order_no,
        "Amount" => $sum,
        "Currency" => 933,
        "Surname" => $order->billing["lastname"],
        "FirstName" => $order->billing["firstname"],
        "Patronymic" => $order->billing["secondname"],
        "City" => $order->billing["city"],
        "IsNameEditable" => MODULE_PAYMENT_EXPRESSPAYERIP_ISNAMEEDIT ? '1' : '0',
        "IsAddressEditable" => MODULE_PAYMENT_EXPRESSPAYERIP_ISADRESSEDIT ? '1' : '0',
        "IsAmountEditable" => MODULE_PAYMENT_EXPRESSPAYERIP_ISAMOUNTEDIT ? '1' : '0',
        "ReturnType" => "redirect",
        "ReturnUrl" => vam_href_link('expresspayerip_result.php?param=success&amount='. $sum),
        "FailUrl" => vam_href_link('expresspayerip_result.php?param=cancel')
      );

      $secret_word = MODULE_PAYMENT_EXPRESSPAYERIP_USESIGNATURE == "True" ? MODULE_PAYMENT_EXPRESSPAYERIP_SECRETWORD : "";

      $request_params['Signature'] = $this->compute_signature($request_params, $secret_word, MODULE_PAYMENT_EXPRESSPAYERIP_TOKEN, 'add_invoice');
...
// unregister session variables used during checkout
    ...
    function check() {
      if (!isset($this->_check)) {
        $check_query = vam_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_EXPRESSPAYERIP_STATUS'");
        $this->_check = vam_db_num_rows($check_query);
      }
      return $this->_check;
    }

    private function getNotifUrl()
    {
      return str_replace('/admin', null, vam_href_link('expresspayerip_notify.php'));
    }

    function install() {
      $gg = $this->getNotifUrl();
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_EXPRESSPAYERIP_STATUS', 'True', '6', '1', 'vam_cfg_select_option(array(\'True\', \'False\'), ', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_EXPRESSPAYERIP_ALLOWED', '', '6', '2', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_EXPRESSPAYERIP_SORT_ORDER', '0', '6', '3', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, use_function, set_function, date_added) values ('MODULE_PAYMENT_EXPRESSPAYERIP_ZONE', '0', '6', '4', 'vam_get_zone_class_title', 'vam_cfg_pull_down_zone_classes(', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, use_function, date_added) values ('MODULE_PAYMENT_EXPRESSPAYERIP_ORDER_STATUS_ID', '0', '6', '5', 'vam_cfg_pull_down_order_statuses(', 'vam_get_order_status_name', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_EXPRESSPAYERIP_ISTEST', 'True', '6', '6', 'vam_cfg_select_option(array(\'True\', \'False\'), ', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_EXPRESSPAYERIP_SERVICEID', '4', '6', '7', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_EXPRESSPAYERIP_TOKEN', 'a75b74cbcfe446509e8ee874f421bd66', '6', '8', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_EXPRESSPAYERIP_USESIGNATURE', 'True', '6', '9', 'vam_cfg_select_option(array(\'True\', \'False\'), ', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_EXPRESSPAYERIP_SECRETWORD', 'sandbox.expresspay.by', '6', '10', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_EXPRESSPAYERIP_NOTIFURL', '".$gg."', '6', '11', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_EXPRESSPAYERIP_USESIGNATUREFORNOTIF', 'False', '6', '12', 'vam_cfg_select_option(array(\'True\', \'False\'), ', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_EXPRESSPAYERIP_SECRETWORDFORNOTIF', '', '6', '13', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_EXPRESSPAYERIP_SHOWQRCODE', 'True', '6', '14', 'vam_cfg_select_option(array(\'True\', \'False\'), ', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_EXPRESSPAYERIP_PATHTOERIP', '', '6', '15', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_EXPRESSPAYERIP_ISNAMEEDIT', 'True', '6', '16', 'vam_cfg_select_option(array(\'True\', \'False\'), ', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_EXPRESSPAYERIP_ISADRESSEDIT', 'True', '6', '17', 'vam_cfg_select_option(array(\'True\', \'False\'), ', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_EXPRESSPAYERIP_ISAMOUNTEDIT', 'True', '6', '18', 'vam_cfg_select_option(array(\'True\', \'False\'), ', now())");
    }
    ...
    function keys() {
      return array('MODULE_PAYMENT_EXPRESSPAYERIP_STATUS', 'MODULE_PAYMENT_EXPRESSPAYERIP_ALLOWED', 
      'MODULE_PAYMENT_EXPRESSPAYERIP_SORT_ORDER', 'MODULE_PAYMENT_EXPRESSPAYERIP_ZONE', 
      'MODULE_PAYMENT_EXPRESSPAYERIP_ORDER_STATUS_ID', 'MODULE_PAYMENT_EXPRESSPAYERIP_ISTEST', 
      'MODULE_PAYMENT_EXPRESSPAYERIP_SERVICEID', 'MODULE_PAYMENT_EXPRESSPAYERIP_TOKEN', 'MODULE_PAYMENT_EXPRESSPAYERIP_USESIGNATURE',
      'MODULE_PAYMENT_EXPRESSPAYERIP_SECRETWORD','MODULE_PAYMENT_EXPRESSPAYERIP_NOTIFURL','MODULE_PAYMENT_EXPRESSPAYERIP_USESIGNATUREFORNOTIF',
      'MODULE_PAYMENT_EXPRESSPAYERIP_SECRETWORDFORNOTIF', 'MODULE_PAYMENT_EXPRESSPAYERIP_SHOWQRCODE', 'MODULE_PAYMENT_EXPRESSPAYERIP_PATHTOERIP', 
      'MODULE_PAYMENT_EXPRESSPAYERIP_ISNAMEEDIT', 'MODULE_PAYMENT_EXPRESSPAYERIP_ISADRESSEDIT', 'MODULE_PAYMENT_EXPRESSPAYERIP_ISAMOUNTEDIT');
    }
